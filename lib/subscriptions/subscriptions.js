  ...
  process: function() {
    if(Meteor.isServer) {
      ...
      if(this.pastDue()) {
        ...
        let nextCharge;
        if(this.lastCartSpawnedAt) {
          nextCharge = new Date()
        } else {
          nextCharge = moment(this.startAt).add(1, 'months').toDate();
        }
        Mart.Subscriptions.update(this._id, {$set: {
          lastCartSpawnedAt: nextCharge
        }})
      }
    }
  },
      ...
      if((now >= this.startAt.getTime()) && (this.state === Mart.Subscription.STATES.SPAWNED)) {
        ...
        if(cart) {
          ...
          try {
            ...
            console.log('created subscription w/ strip');
            const up = Mart.Subscriptions.update(this._id, {$set: {
