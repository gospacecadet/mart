Meteor.publish('mart/cart', function() {
  ...
  cartHandle = Mart.Carts.find({userId: userId, state: Mart.Cart.STATES.SHOPPING}).observeChanges({
...
Meteor.methods({
  'mart/cart/findCurrentOrCreate': function() {
    console.log('mart/cart/findCurrentOrCreate');
...
// Mart.Carts.permit(['update']).if
//
// Security.defineMethod("inCurrentState", {
//   fetch: [],
//   transform: null,
