Meteor.publish('mart/cart', function(cartId) {
  check(cartId, String);
  if(this.userId) {
    return Mart.Carts.find({_id: cartId, userId: this.userId})
  }
  this.ready();
})
...
Meteor.publish('mart/_cart', function(lineItemId) {
  ...
  return [
    Mart.Carts.find({_id: cart._id}),
    Mart.LineItems.find({_id: lineItemId}),
    Mart.Products.find({_id: lineItem.productId}),
    Mart.Storefronts.find({_id: product.storefrontId}),
    Mart.Images.find({$or: [
      {objectCollection: "Storefronts", objectId: product.storefrontId},
      {objectCollection: "Products", objectId: product._id},
    ]}),
    Mart.Prices.find({productId: product._id})
  ]
});
