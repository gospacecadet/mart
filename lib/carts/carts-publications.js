...
Meteor.publish('mart/carts', function(states, guestId) {
  ...
  if(Roles.userIsInRole(this.userId, // admin
    ...
    Mart.ROLES.GROUPS.GLOBAL)) {
      ...
      var ownedStorefronts = Mart.Storefronts.find({
        userId: this.userId
      }).fetch()

      var ownedStorefrontIds = _.map(ownedStorefronts, function(storefront) {
        return storefront._id
      })

      cartsCursor = Mart.Carts.find({storefrontId: {$in: ownedStorefrontIds}})
