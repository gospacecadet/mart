_.extend(Cart.prototype, {
  machina: function() {
    ...
    fsm.states[Mart.Cart.STATES.WAITING_TRANSFER_ACCEPTANCE] = {
      ...
      "settleOffline": Mart.Cart.STATES.SETTLED_OFFLINE,
      _onExit: function() {
      }
    }
    ...
    ///////////////////
    // SETTLED OFFLINE (check sent, wire transfer, etc)
    ///////////////////
    fsm.states[Mart.Cart.STATES.SETTLED_OFFLINE] = {
      _onEnter: function() {
        Mart.Carts.update(this._._id, {$set: {
          state: Mart.Cart.STATES.SETTLED_OFFLINE,
          settledAt: new Date()
        }})
      },
      _onExit: function() {
  ...
  settleOffline() {
    this.machina().handle('settleOffline')
  }
})
