...
Meteor.methods({
  ...
  'mart/stripe/activate-bank-account': function(bankAccountId) {
    // console.log('mart/stripe/activate-bank-account');
    check(bankAccountId, String);

    var bankAccount = Mart.BankAccounts.findOne({_id: bankAccountId, userId: Meteor.userId()})
    if(bankAccount) {
      var managedAccount = Mart.GatewayTypes.Stripe.ManagedAccounts.findOne({userId: Meteor.userId()})

      var Stripe = StripeAPI(Mart.STRIPE_SECRET_KEY)
      var updateExternalAccount = Meteor.wrapAsync(Stripe.accounts.updateExternalAccount, Stripe.accounts);

      try {
        var result = updateExternalAccount(
          managedAccount.stripeToken,
          bankAccount.gatewayToken,
          {default_for_currency: true}
        );

        // Reset current default and set this
        if(result.default_for_currency) {
          Mart.BankAccounts.update({isDefault: true}, {$set: {isDefault: false}})
          Mart.BankAccounts.update(bankAccountId, {$set: {isDefault: true}})
          return true
        }
        return false
      } catch (error) {
        console.log(error);
        throw new Meteor.Error("stripe-activate-bank-account-error", error.message);
      }
    }
  }
});
